...
export class AlphaVantageFetcherService {
  ...
  private async getMinuteHistory({
    symbol,
    full,
  }: {
    symbol: string;
  ...
  private async getHourlyHistory({
    symbol,
    full,
  }: {
    symbol: string;
  ...
  public async getHistoricData({
    ...
    period,
    historyLength,
  }: HistoricDataRequest): Promise<HistoricData> {
    let alphaRes: any;
    switch (period) {
      case Period.MINUTE:
        alphaRes = await this.getMinuteHistory({
          symbol: symbol,
          full: historyLength > 100,
        });
        return {
          period: period,
          historyLength: historyLength,
          timezone: alphaRes['Meta Data']['9. Time Zone'],
          history: Object.entries(alphaRes['Time Series Crypto (1min)'])
            .map((minuteInfo) => {
              return {
      ...
      case Period.HOURLY:
        alphaRes = await this.getHourlyHistory({
          symbol: symbol,
          full: historyLength > 100,
        });
        return {
          ...
          historyLength: historyLength,
          timezone: alphaRes['Meta Data']['9. Time Zone'],
          history: Object.entries(alphaRes['Time Series Crypto (60min)'])
            .map((hourlyInfo) => {
              return {
      ...
      case Period.DAILY:
        ...
        return {
          ...
          history: Object.entries(
            alphaRes['Time Series (Digital Currency Daily)'],
          ).map((dailyInfo) => {
            return {
