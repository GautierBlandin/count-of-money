...
import {DeleteCryptoCurrencyRequest, GetCryptoCurrencyRequest} from "./crypto-dal/cryptos.interface";
import { Period } from "./external-fetcher/crypto-external-fetcher.interface"

@Injectable()
export class CryptosService {
    constructor(private cryptoDal: CryptoDalService,
    ...
    public async getDynamicCrypto(req: GetCryptoCurrencyRequest): Promise<GetCryptoResponse>{
    ...
    public async getCryptoHistory(req: {symbol: string, period: string}): Promise<GetRealTimeDataResponse>{
        switch(req.period){
            case 'minute':
    ...
    public async createCrypto(req: {geckoID: string}): Promise<GetCryptoResponse>{
